...
/**
 * @brief A C++17 thread pool class. The user submits tasks to be executed into a queue_. Whenever a thread becomes available, it pops a task from the queue_ and executes it. Each task is automatically assigned a future, which can be used to wait for the task to finish executing and/or obtain its eventual return value.
 */
class thread_pool
{
...
public:
    ...
    /**
     * @brief Destruct the thread pool. Waits for all tasks to complete, then destroys all threads. Note that if the variable paused is set to true, then any tasks still in the queue_ will never be executed.
     */
    ~thread_pool()
    {
        wait_for_tasks();
    ...
    /**
     * @brief Get the number of tasks currently waiting in the queue_ to be executed by the threads.
     *
    ...
    /**
     * @brief Get the total number of unfinished tasks - either still in the queue_, or running in a thread.
     *
    ...
    /**
     * @brief Push a function with no arguments or return value into the task queue_.
     *
    ...
    /**
     * @brief Push a function with arguments, but no return value, into the task queue_.
     * @details The function is wrapped inside a lambda in order to hide the arguments, as the tasks in the queue_ must be of type std::function<void()>, so they cannot have any arguments or return value. If no arguments are provided, the other overload will be used, in order to avoid the (slight) overhead of using a lambda.
     *
     * @tparam F The type of the function.
    ...
    /**
     * @brief Reset the number of threads in the pool. Waits for all currently running tasks to be completed, then destroys all threads in the pool and creates a new thread pool with the new number of threads. Any tasks that were waiting in the queue_ before the pool was reset will then be executed by the new threads. If the pool was paused before resetting it, the new pool will be paused as well.
     *
     * @param _thread_count The number of threads to use. The default value is the total number of hardware threads available, as reported by the implementation. With a hyperthreaded CPU, this will be twice the number of CPU cores. If the argument is zero, the default value will be used instead.
     */
    ...
    /**
     * @brief Submit a function with zero or more arguments and no return value into the task queue_, and get an std::future<bool> that will be set to true upon completion of the task.
     *
     * @tparam F The type of the function.
    ...
    /**
     * @brief Submit a function with zero or more arguments and a return value into the task queue_, and get a future for its eventual returned value.
     *
     * @tparam F The type of the function.
     * @tparam A The types of the zero or more arguments to pass to the function.
    ...
    /**
     * @brief Wait for tasks to be completed. Normally, this function waits for all tasks, both those that are currently running in the threads and those that are still waiting in the queue_. However, if the variable paused is set to true, this function only waits for the currently running tasks (otherwise it would wait forever). To wait for a specific task, use submit() instead, and call the wait() member function of the generated future.
     */
    void wait_for_tasks()
    {
        while (true)
    ...
    /**
     * @brief An atomic variable indicating to the workers to pause. When set to true, the workers temporarily stop popping new tasks out of the queue_, although any tasks already executed will keep running until they are done. Set to false again to resume popping tasks.
     */
    std::atomic<bool> paused = false;

    /**
     * @brief The duration, in microseconds, that the worker function should sleep for when it cannot find any tasks in the queue_. If set to 0, then instead of sleeping, the worker function will execute std::this_thread::yield() if there are no tasks in the queue_. The default value is 1000.
     */
...
private:
    ...
    /**
     * @brief Try to pop a new task out of the queue_.
     *
     * @param task A reference to the task. Will be populated with a function if the queue_ is not empty.
     * @return true if a task was found, false if the queue_ is empty.
     */
    ...
    /**
     * @brief A worker function to be assigned to each thread in the pool. Continuously pops tasks out of the queue_ and executes them, as long as the atomic variable running is set to true.
     */
    void worker()
    {
        while (running)
    ...
    /**
     * @brief A mutex to synchronize access to the task queue_ by different threads.
     */
    ...
    /**
     * @brief A queue_ of tasks to be executed by the threads.
     */
    ...
    /**
     * @brief An atomic variable to keep track of the total number of unfinished tasks - either still in the queue_, or running in a thread.
     */
